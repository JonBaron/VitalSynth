...
public sealed class SlowSignals(Cfg cfg)
{
    ...
    public Percent LastSpo2  { get; private set; } = cfg.Spo2Base;
    public Celsius LastTempC { get; private set; } = cfg.TempBase;

    public double Respiration(double dt)
    {
        _respPhase += 2 * Math.PI * cfg.RespiratoryRate.Value / 60 * dt;
        return Math.Sin(_respPhase);
    }

    public Percent AdvanceSpo2(double dt)
    {
        _spo2Walk = Math.Clamp(_spo2Walk + (_rng.NextDouble() - 0.5) * cfg.Spo2Drift * dt, -1.5, 1.5);
        var value = cfg.Spo2Base.Value + _spo2Walk;
        if (cfg.Scenario is Scenario.Desaturation)
            value -= 8 * (1 - Math.Exp(-_clock.Elapsed.TotalSeconds / 15));
        LastSpo2 = new Percent(Math.Clamp(value, 70, 100));
        return LastSpo2;
    }

    public Celsius AdvanceTemperature(double dt)
    {
        _tempWalk += (_rng.NextDouble() - 0.5) * cfg.TempDrift * dt;
        LastTempC = Celsius.Clamp(cfg.TempBase.Value + _tempWalk);
        return LastTempC;
    }
}
